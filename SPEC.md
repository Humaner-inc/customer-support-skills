...
- `dist/catalog.json` / `dist/catalog.js` -- full catalog (includes
  problem-solving procedure markdown)
- `dist/runtime.js` + `dist/runtime.d.ts` -- **slim agent entry** with
  baseline skills only (core / behavior / escalation / guardrails). Prefer
  `@humaner/customer-support-skills/runtime` on chat / prompt hot paths.
- `dist/industries/<id>.js` (+ `<id>.runtime.js`) -- per-industry modules
  for tree-shaking / lazy loads
- `dist/index.js` + `dist/index.d.ts` -- full typed accessor API
  (`getIndustry(id)`, `listIndustries()`, `listIndustryIds()`,
  `SKILLS_VERSION`)

Consumers (like Humaner's private runtime) depend on this package and adapt
the catalog into their own internal types. The build output is never
